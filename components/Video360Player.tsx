...
export default function Video360Player({ videoUrl, title }: Video360PlayerProps) {
    ...
    useEffect(() => {
        ...
        video.loop = true;
        video.muted = false; // 초기 상태 음소거 false
        video.volume = volume;
        video.playsInline = true;
        video.preload = 'metadata'; // 메타데이터만 먼저 로드
        video.autoplay = false; // 자동재생 false
        ...
        video.addEventListener('loadeddata', () => {
            ...
            playButton.className = 'absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-blue-600 text-white p-4 rounded-full hover:bg-blue-700 transition-colors shadow-lg';
        ...
        <div ref={wrapperRef} className="w-full aspect-video rounded-lg overflow-hidden bg-gray-100 dark:bg-neutral-800 mb-6 relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <div className="flex items-center gap-4 text-white">
                    ...
                    <button
                        onClick={togglePlay}
                        className="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full hover:bg-white/30 transition flex-shrink-0"
                    >
                        {isPlaying ? (
                    ...
                    {isVRSupported && (
                        <button
                            ...
                            className={`w-8 h-8 flex items-center justify-center rounded-full transition flex-shrink-0 ${isVRMode
